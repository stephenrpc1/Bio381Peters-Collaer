...
# Global Variables -------------
df1 <- create_simdata(slope=2.5, n=20, mean=1.8, sd=0.7)
...
```{r, Question 2}
# Description ------------------
# A script that simulates data for my yellow birch regeneration project.  This script creates simulated data based on the real parameters from my dataset and then runs a linear model and graphs the results.  The independent variable is the size of the fallen over root mounds that the yellow brich are growing on and the dependent variable is the count of yellow birch seedlings growing on the root mound.
...
# ------------------------
create_simdata <- function(slope = runif(1), n = 10, mean = 50, sd = 10, intercept = 0) {
  noise <- round(rnorm(n = n, mean = 0, sd = 5))
  x_val  <- round(rnorm(n = n, mean = mean, sd = sd))
  y_val <- slope*x_val + noise + intercept
  df <- data.frame(x_val, y_val)
...
# purpose: Use x and y vectors to create a linear model
# input:x and y vectors of the same length 
# output: A summary of the coefficients from a linear model run on these vectors
# ------------------------
run_model <- function(x= NULL, y = NULL) {
  if(is.null(x)) print("No Data!") else
  lm1 <- lm(x ~ y)
  return(summary(lm1)$coefficients)
}
...
# Global Variables -------------
df1 <- create_simdata(n = 20, mean = 7.4, sd = 10.9) #these are the updated values for the count of seedlings
